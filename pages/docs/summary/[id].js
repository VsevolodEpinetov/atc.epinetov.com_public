...
// libraries
import { getAllSummaryDocsIds, getDocSummaryData } from 'lib/docsSummary'
...
export default function DocPage({ docData }) {
  ...
  const navigationLinks = (docData) => {
    var summaryLink = <Link href={`/docs/${docData.id}`}><a>🔍 Полная версия</a></Link>
...
export async function getStaticPaths() {
  const paths = getAllSummaryDocsIds()
  return {
    paths,
    fallback: false
  }
...
export async function getStaticProps({ params }) {
  const docData = await getDocSummaryData(params.id)
  return {
    props: {
      docData
    }
